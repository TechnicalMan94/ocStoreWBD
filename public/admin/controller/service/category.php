...
class ControllerServiceCategory extends Controller {
	...
	private function getCategories($parent_id, $parent_path = '', $indent = '') {
		...
		if ($href_category === null) {
			...
			$href_action = $this->url->link('service/category/update', 'user_token=' . $this->session->data['user_token'] . '&service_category_id=', true);
		}
		...
		foreach ($results as $result) {
			...
			$action[] = array(
				...
				'href' => $href_action . $result['service_category_id']
			);
			$output[$result['service_category_id']] = array(
				...
				'action'      => $action,
				'href'        => $href,
				'href_shop'   => HTTP_CATALOG . 'index.php?route=service/category&service_category_id=' . ($result['service_category_id']),
				'indent'      => $indent
			);
			if ($service_category_id == $result['service_category_id']) {
