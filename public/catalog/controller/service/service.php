...
class ControllerServiceService extends Controller {
	...
	public function index() { 
		...
		if (isset($this->request->get['service_category_id'])) {
			...
			foreach (explode('_', $this->request->get['service_category_id']) as $path_id) {
				if (!$service_category_id) {
					$service_category_id = $path_id;
				} else {
					$service_category_id .= '_' . $path_id;
				}
				...
				if ($category_info) {
					$data['breadcrumbs'][] = array(
						...
						'href'      => $this->url->link('service/category', 'service_category_id=' . $service_category_id)
					);
				}
			}
		}
	...
	if (isset($this->request->get['filter_name']) || isset($this->request->get['filter_tag'])) {
		...
		if ($service_info) {
			...
			$data['breadcrumbs'][] = array(
				...
				'href' => $this->url->link('service/service', 'service_id=' . $this->request->get['service_id'])
			);
			...
			if ($service_info['noindex'] <= 0 && $this->config->get('config_noindex_status')) {
				$this->document->setRobots('noindex,follow');
			}

			$this->document->setDescription($service_info['meta_description']);
			$this->document->setKeywords($service_info['meta_keyword']);
			$this->document->addLink($this->url->link('service/service', 'service_id=' . $this->request->get['service_id']), 'canonical');
			...
			$data['service_review'] = (int)$service_info['service_review'];
			$data['reviews'] = sprintf($this->language->get('text_reviews'), (int)$service_info['reviews']);
			$data['rating'] = (int)$service_info['rating'];
			$data['gstatus'] = (int)$service_info['gstatus'];
			$data['description'] = html_entity_decode($service_info['description'], ENT_QUOTES, 'UTF-8');
			$data['fields'] = $service_info['fields'];
			...
			foreach ($results as $result) {
				...
				$data['services'][] = array(
					...
					'href'    	 => $this->url->link('service/service', 'service_id=' . $result['service_id']),
				);
			}
			...
			$results = $this->model_service_service->getServiceRelatedProduct($this->request->get['service_id']);
			...
			$data['download_status'] = false;
			
			$data['downloads'] = array();
			
			$results = $this->model_service_service->getDownloads($this->request->get['service_id']);
            ...
            foreach ($results as $result) {
                if (file_exists(DIR_DOWNLOAD . $result['filename'])) {
                    ...
                    $data['downloads'][] = array(
                        ...
                        'href'       => $this->url->link('service/service/download', '&service_id='. $this->request->get['service_id']. '&download_id=' . $result['download_id'])
                    );
                }
            } 
			
			$this->model_service_service->updateViewed($this->request->get['service_id']);
			...
			if (isset($this->request->get['filter_news_id'])) {
				...
				'href' => $this->url->link('product/product', $url . '&product_id=' . $service_id)
			);
	...
	public function download() {
		...
		$download_info = $this->model_service_service->getDownload($service_id, $download_id);
	...
	public function write() {
		$json = array('error' => 'Reviews are disabled for services.');
