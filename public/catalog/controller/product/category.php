	...
	public function index() {
		...
		if ($category_info) {
			...
			if ($category_info['image']) {
				$data['thumb'] = $this->model_tool_image->resize($category_info['image'], $this->config->get('theme_default_image_category_width'), $this->config->get('theme_default_image_category_height'));
			} else {
				$data['thumb'] = '';
			}
			...
			foreach ($results as $result) {
				...
				$data['products'][] = array(
					...
					'href'        => $this->url->link('product/product', 'path=' . $this->request->get['path'] . '&product_id=' . $result['product_id'] . (!empty($result['variant_key']) ? '&variant_key=' . $result['variant_key'] : '') . $url)
				);
			}
